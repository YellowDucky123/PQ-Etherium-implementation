...
struct TweakableHash {
    ...
    /// Applies the tweakable hash to parameter, tweak, and a list of messages
    /// (1 element for a chain step, 2 for a tree node, many for a leaf).
    virtual Domain apply(Parameter parameter, Tweak& tweak, std::vector<Domain>& messages) = 0;

    virtual void internal_consistency_check() = 0;
};

template <typename TH>
typename TH::Domain chain(TH &th, const typename TH::Parameter &parameter,
     uint32_t epoch, uint8_t chain_index, uint8_t start_pos_in_chain, uint steps, const typename TH::Domain &start) {
    using TH_domain = typename TH::Domain;
    
    TH_domain current = start;

    for(uint j = 0; j < steps; j++) {
        auto tweak = th.chain_tweak(epoch, chain_index, start_pos_in_chain + static_cast<uint8_t>(j) + static_cast<uint8_t>(1));
        std::vector<TH_domain> message{current};
        current = th.apply(parameter, *tweak, message);
    }
